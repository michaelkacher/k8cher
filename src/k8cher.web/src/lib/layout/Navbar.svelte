...
>
	<div class="flex items-center justify-between px-4 py-3 sm:p-0">
		<div class="flex items-baseline justify-between text-gray-100">
			<a href="/" on:click="{() => (isToolbarOpen = false)}"
				>
				<!-- todo - mbk: add back another logo -->
				<!-- <img
					width="48px"
					height="20px"
					class="w-12 m-auto"
			...
			<div data-test="svlAppVersion" class="ml-2 text-xs text-gray-400">
				<!-- todo - mbk: make this populated by environment -->
				DEV
			</div>
		</div>
	...
	<div class:hidden="{!isToolbarOpen}" class="sm:flex sm:items-center">
		...
		<!-- todo - mbk: add back auth, but consider flow best for this app -->
		<!-- let them work to start, then to save content enter e-mail and click 'verify' -->
		<!-- once verified, user, data associated to e-mail -->
